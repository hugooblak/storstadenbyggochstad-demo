...
add_filter(
	...
	function ( $exporters ) {
		$exporters['storstadenbyggochstad-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'storstadenbyggochstad-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'storstadenbyggochstad-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'storstadenbyggochstad-leads',
						'group_label' => __( 'Quote requests', 'storstadenbyggochstad-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['storstadenbyggochstad-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'storstadenbyggochstad-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
