...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'storstadenbyggochstad-leads' ),
					'singular_name' => __( 'Lead', 'storstadenbyggochstad-leads' ),
					'edit_item'     => __( 'Förfrågan', 'storstadenbyggochstad-leads' ),
					'search_items'  => __( 'Search leads', 'storstadenbyggochstad-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'storstadenbyggochstad-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'storstadenbyggochstad-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'storstadenbyggochstad-leads' ),
			'npl_phone'    => __( 'Phone', 'storstadenbyggochstad-leads' ),
			'npl_service'  => __( 'Needs', 'storstadenbyggochstad-leads' ),
			'npl_timeline' => __( 'When', 'storstadenbyggochstad-leads' ),
			'npl_zip'      => __( 'ZIP', 'storstadenbyggochstad-leads' ),
			'npl_source'   => __( 'Source', 'storstadenbyggochstad-leads' ),
			'npl_status'   => __( 'Status', 'storstadenbyggochstad-leads' ),
			'date'         => __( 'Received', 'storstadenbyggochstad-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'storstadenbyggochstad-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'storstadenbyggochstad-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'storstadenbyggochstad-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'storstadenbyggochstad-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'storstadenbyggochstad-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'storstadenbyggochstad-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'storstadenbyggochstad-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'storstadenbyggochstad-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'storstadenbyggochstad-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'storstadenbyggochstad-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'storstadenbyggochstad-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'storstadenbyggochstad-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
