<?php
/**
 * Storstaden Bygg & Städservice AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Storstaden Bygg & Städservice AB Leads" plugin, so leads survive a theme change.
 *
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'storstadenbyggochstad', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'storstadenbyggochstad' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'storstadenbyggochstad' ) ) );

		register_block_pattern_category( 'storstadenbyggochstad-sections', array( 'label' => __( 'Storstaden Bygg & Städservice AB sections', 'storstadenbyggochstad' ) ) );
		register_block_pattern_category( 'storstadenbyggochstad-pages', array( 'label' => __( 'Storstaden Bygg & Städservice AB page layouts', 'storstadenbyggochstad' ) ) );
	}
);
